...
def run_backtest(df, name, config):
    ...
    simulator.plot_hpr(path=f"result/backtest/{file_prefix}_hpr.png")
    simulator.plot_drawdown(path=f"result/backtest/{file_prefix}_drawdown.png")
    simulator.plot_inventory(path=f"result/backtest/{file_prefix}_inventory.png")
    
    simulator.plot_price_with_z_thresholds(df, threshold=1.5, path=f"result/backtest/{file_prefix}_price_bands.png", time_col="datetime")
    print(f"Charts saved in the 'result/backtest' folder.")

def main():
    # Load parameters from JSON
    config_path = 'parameter/backtesting_parameter.json'
    try:
